...
ENGINE_VERSION = "backtest_v33_edge_ranking"
TICKER = "ETC-USD"
INTERVAL = "1h"
PERIOD = "730d"
...
def main() -> None:
    ...
    feature_tables = {
        "Signal": trade_ledger.groupby("entry_signal", observed=True)["pnl_pct"].mean(),
        "Direction": trade_ledger.groupby("direction", observed=True)["pnl_pct"].mean(),
        "Range": trade_ledger.groupby("entry_range_bucket", observed=True)["pnl_pct"].mean(),
        "MA Distance": trade_ledger.groupby("ma_bucket", observed=True)["pnl_pct"].mean(),
        "Hold Time": trade_ledger.groupby("bars_held", observed=True)["pnl_pct"].mean(),
    }
