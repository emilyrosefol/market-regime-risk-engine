...
ENGINE_VERSION = "backtest_v36_winner_analysis"
TICKER = "ETC-USD"
...
def run_atr_execution(
    ...
    for i in range(1, len(df)):
        ...
        if signal != 0:
            ...
            # Smaller entry candles receive more points
            if entry_range_pct <= 0.005:
                edge_score += 30
            elif entry_range_pct <= 0.010:
                edge_score += 20
            ...
            # Moderate MA distance receives the most points
            if 0.002 <= ma_distance_pct <= 0.010:
                edge_score += 25
            elif ma_distance_pct < 0.002:
                edge_score += 15
