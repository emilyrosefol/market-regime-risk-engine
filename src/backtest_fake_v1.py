...
if __name__ == "__main__":
        ...
        df["vol_ok"] = df["atr_pct"] > df["atr_pct_mean50"]
...
df.loc[fake_break_long & (df["rsi"] < LONG_RSI), "decision"] = "GO_FAKE_LONG"

df.loc[fake_break_short & (df["rsi"] > SHORT_RSI), "decision"] = "GO_FAKE_SHORT"
...
print(df[["position", "position_change"]].tail(10))
df["trade_start"] = ((df["position"] != 0) & (df["position_change"] != 0)).astype(int)
print(df[["position", "position_change", "trade_start"]].tail(15))
df["trade_id"] = df["trade_start"].cumsum()
df.loc[df["position"] == 0, "trade_id"] = 0
print(df[["position", "trade_start", "trade_id"]].tail(20))
...
for trade_id, grp in df[df["trade_id"] > 0].groupby("trade_id"):
    entry_row = grp.iloc[0]
    exit_row = grp.iloc[-1]

    direction = "LONG" if entry_row["position"] == 1 else "SHORT"
    entry_price = float(entry_row["Close"])
    exit_price = float(exit_row["Close"])
    bars_held = int(len(grp))
    ...
    trade_rows.append({
        ...
        "entry_time": entry_row["Datetime"] if "Datetime" in df.columns else entry_row.name,
        "exit_time": exit_row["Datetime"] if "Datetime" in df.columns else exit_row.name,
        "direction": direction,
...
print("\n=== TRADE LOG SUMMARY ===")
print("Total completed trades:", len(trade_log))

if len(trade_log) > 0:
    print("Win rate %:", (trade_log["trade_return"] > 0).mean() * 100)
    print("Average trade return %:", trade_log["trade_return"].mean() * 100)
    print("Average bars held:", trade_log["bars_held"].mean())
    print("\nLast 10 trades:")
    print(trade_log.tail(10))
else:
    print("No completed trades found.")

trade_log.to_csv("analysis/trade_log_fake_v1.csv", index=False)
...
print("Time:", last.name)
print("Decision:", last["decision"])
print("Signal:", last["signal"])
print("Position:", last["position"])
