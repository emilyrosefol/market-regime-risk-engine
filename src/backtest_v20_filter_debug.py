...
ENGINE_VERSION = "v20_filter_debug"
TICKER = "GOOG"
TICKER = "BTC-USD"
INTERVAL = "1h"
PERIOD = "730d"
...
def run_atr_execution(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for i in range(1, len(df)):
        ...
        if signal != 0:
            ...
            if entry_signal == "GO_RANGE_LONG":
                print("FOUND GO_RANGE_LONG")
                print("Bar:", i)
                print(df.iloc[i][["decision", "signal", "trend_dir", "regime", "rsi"]])


            allowed_signal = entry_signal in ["GO_FAKE_SHORT", "GO_RANGE_SHORT"]
