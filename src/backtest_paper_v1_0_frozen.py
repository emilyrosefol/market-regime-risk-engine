...
def build_trade_log(result: pd.DataFrame) -> pd.DataFrame:
    """
    Build a trade log purely from position flips.
    Entry = position changes 0 -> +/-1
    Exit  = position changes +/-1 -> 0
    """
    df = result.copy()
    ...
    # Entry when we go flat -> in a position
    entry_sig = (pos_prev == 0.0) & (pos != 0.0)

    # Exit when we go in a position -> flat
    exit_sig = (pos_prev != 0.0) & (pos == 0.0)

    # We'll price entries/exits at Open of the bar where flip happens (simple + consistent)
    # Pick a price column that exists (prefer Open, fall back to close)
    if "Open" in df.columns:
        open_ = df["Open"].astype(float)
    elif "open" in df.columns:
        open_ = df["open"].astype(float)
    elif "close" in df.columns:
        open_ = df["close"].astype(float)
    elif "Close" in df.columns:
        open_ = df["Close"].astype(float)
    else:
        raise ValueError(f"No price column found. Columns are: {list(df.columns)}")
    ...
    for i in range(len(df)):
        ...
        # If we are in a trade, check for exit
        if exit_sig.iloc[i]:
            exit_price = float(open_.iloc[i])
            entry_price = float(cur["entry_price"])
            direction = cur["direction"]

            # pnl% convention: long = (exit/entry - 1), short = (entry/exit - 1)
            if direction == "LONG":
    ...
    if len(tdf) == 0:
        ...
        return pd.DataFrame(columns=["trade_id","direction","entry_idx","entry_price","exit_idx","exit_price","pnl","pnl_pct","win"])

    tdf["pnl_pct"] = tdf["pnl"] * 100.0
    tdf["win"] = tdf["pnl"] > 0
    return tdf

    def print_latest_signal(result: pd.DataFrame, account_size: float = 10_000.0, risk_unit: float = 0.01) -> None:
        last = result.iloc[-1]

        close = float(last.get("close", last.get("Close", 0.0)))
        regime = str(last.get("regime", "UNKNOWN"))
        decision = str(last.get("decision", "WAIT"))
        trend_dir = int(last.get("trend_dir", 0))
        position = float(last.get("position", 0.0))
        ...
        print(f"New trade now?: {new_trade}")
    ...
    account_size = 10000
    risk_dollars = account_size * RISK_UNITError("result must include 'position' and 'strategy_return'")
...
def run_backtest(price: pd.DataFrame) -> pd.DataFrame:
    ...
    trend_strength = (fast - slow).abs() / atr.replace(0, np.nan)
    trend_strength_ok = trend_strength > MIN_TREND_STRENGTH_ATR
...
def print_latest_signal(result: pd.DataFrame):
    ...
    # grab price no matter what the column name is
    close = last.get("Close", last.get("close", last.get("price", None)))

    regime = last.get("regime", "UNKNOWN")
    decision = last.get("decision", "WAIT")
    trend_dir = int(last.get("trend_dir", 0))
    position = float(last.get("position", 0.0))

    # risk: prefer a computed column, otherwise fall back to your constant if you have it
    risk_dollars = last.get("risk_dollars", None)
    if risk_dollars is None:
        try:
            risk_dollars = float(RISK_DOLLARS)  # if you defined RISK_DOLLARS somewhere
        except Exception:
            risk_dollars = 0.0
    ...
    if position > 0:
        ...
        if str(decision) == "GO" and trend_dir == 1:
            status = "GO LONG"
        elif str(decision) == "GO" and trend_dir == -1:
            status = "GO SHORT"
        else:
            status = "WAIT"
    ...
    print("ENGINE DAILY SIGNAL")
    print("==============================")
    print(f"Price:    {close:.2f}" if close is not None else "Price:    N/A")
    print(f"Regime:   {regime}")
    print(f"Decision: {decision}")
    print(f"TrendDir: {trend_dir}")
    print(f"Position: {position}")
    print(f"Action:   {status}")
    print(f"Risk:     ${float(risk_dollars):.2f}")
    print("==============================\n")
...
def main():
    ...
    # ---- COSTS / SLIPPAGE (turnover-based) ----
    FEE_BPS = 2.0
    SLIPPAGE_BPS = 3.0

    pos = result["position"].astype(float).fillna(0.0)
    turnover = pos.diff().abs().fillna(0.0)

    cost_per_turn = (FEE_BPS + SLIPPAGE_BPS) / 10_000.0
    cost = turnover * cost_per_turn

    result["strategy_return_net"] = result["strategy_return"].fillna(0.0) - cost
    result["equity_curve_net"] = (1.0 + result["strategy_return_net"]).cumprod()
    ...
    def strat_stats(df: pd.DataFrame):
        eq = (1.0 + df["strategy_return_net"]).cumprod()
    ...
    strat = (result["equity_curve_net"].iloc[-1] - 1.0) * 100.0
    print(f"\nBuy&Hold %: {buy_hold:.2f}%")
    print(f"Strategy NET %: {strat:.2f}%")
    ...
    if trade_log.empty:
        ...
        if not trade_log.empty:
            ...
            def summarize(name, df):
                ...
                # choose whichever pnl column exists
                if "pnl_pct" in df.columns:
                    ...
                    pnl = df["pnl"]
                    unit = ""
                elif "pnl_sum" in df.columns:
                    pnl = df["pnl_sum"]
                    unit = ""
                else:
                    print(f"{name}: Trades={len(df)} (no pnl column found)")
                ...
                print(f" Avg trade {unit}: {avg_trade:.3f}")
                print(f" Total return {unit}: {total_return:.2f}")
                print()
    ...
    print("\n[DEBUG] result columns:", list(result.columns))
    print("[DEBUG] last row keys:", list(result.iloc[-1].index))
    print("[DEBUG] last row snapshot:", result.iloc[-1].to_dict())
