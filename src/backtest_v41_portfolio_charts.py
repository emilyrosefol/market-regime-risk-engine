# ============================
# VERSION: V40 CLEAN ENGINE
# - fake breakouts only
# - range + fake breakout entries
# - ATR stop / take-profit engine
# - compact reporting + optional detailed analysis
...
ENGINE_VERSION = "backtest_v41_portfolio_charts"
TICKER = "ETC-USD"

DEBUG = False
DETAILED_ANALYSIS = False

RUN_MARKET_COMPARISON = True
RUN_YEARLY_VALIDATION = True
...
def save_outputs(df: pd.DataFrame, trade_ledger: pd.DataFrame, equity_curve: pd.DataFrame) -> None:
    ...
    prefix = f"{ENGINE_VERSION}_{safe_ticker}"

    df.to_csv(analysis_dir / f"{prefix}_output.csv", index=False)
    trade_ledger.to_csv(analysis_dir / f"{prefix}_trades.csv", index=False)
    equity_curve.to_csv(analysis_dir / f"{prefix}_equity.csv", index=False)

    print(f"\nSaved CSVs to: {analysis_dir}")
...
def save_charts(
    ...
    """Save portfolio-ready performance charts to the analysis folder."""
    if not SAVE_CHARTS:
        return
    ...
    if not equity_curve.empty:
        ...
        plt.plot(equity_curve["trade_num"], equity_curve["equity"])
        plt.title(f"{TICKER} Equity Curve")
        plt.xlabel("Trade Number")
        plt.ylabel("Account Equity")
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(analysis_dir / f"{prefix}_equity_curve.png", dpi=150)
        plt.close()

        plt.figure(figsize=(10, 6))
        plt.plot(equity_curve["trade_num"], equity_curve["drawdown"] * 100)
        ...
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(analysis_dir / f"{prefix}_drawdown_curve.png", dpi=150)
        plt.close()

    if not comparison_df.empty:
        chart_df = comparison_df.sort_values("return_pct", ascending=False)
        plt.figure(figsize=(10, 6))
        plt.bar(chart_df["market"], chart_df["return_pct"])
        plt.axhline(0, linewidth=1)
        plt.title("Market Return Comparison")
        plt.xlabel("Market")
        plt.ylabel("Return (%)")
        plt.tight_layout()
        plt.savefig(analysis_dir / f"{ENGINE_VERSION}_market_comparison.png", dpi=150)
        plt.close()

    print(f"Saved charts to: {analysis_dir}")
