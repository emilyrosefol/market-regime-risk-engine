...
def main() -> None:
    ...
    print(result["risk_multiplier"].describe())

    active = result[result["position"] != 0]

    print("\nACTIVE position size summary:")
    print(active["position_size"].describe())

    print("\nACTIVE risk multiplier summary:")
    print(active["risk_multiplier"].describe())

    print("\nTrade log total pnl %:")
    print(trade_log["pnl_pct"].sum())

    print("\nStrategy return %:")
    print((result["equity_curve"].iloc[-1] - 1) * 100)
